...
// javascript for easy game 


// tileState holds the game state at any point in time
const tileState = {
  // tileLoc will be set up as a hash of objects of the form {CSS id: tile position} where the locations are numbered 0 to 8.
  // nullLoc is the position of the gap, again numbers 0 through 8. The original code was made for 15 moving pieces; we figured out how to change this 
  tileLoc: {},
  nullLoc: '',
};
...
function gameSetup() {
  ...
  img.onload = function() {
    ...
    const tiles = [...Array(9)].map((_) => tileHTML); // The array is 9 because we want a 3 by 3 puzzle for the easy game
...
function tileSetup() {
  ...
  tileArray.forEach((tile, index) => {
    ...
    const backgroundPositionX = ((index % 3) * 99.6) / 2; // "index %3" and the "/2" portions were changed for our specific board; In the original puzzle they had to be 4 and 3
...
function randomizeBoard() {
  ...
  document.getElementById('next-button').style.display = 'none'; //user can click on it to the next image, but it won't show up at this point
...
//https://www.w3schools.com/jsref/prop_style_display.asp
  ...
  if (checkGameWon()) {
    document.getElementById('randomize-button').style.display = 'block';
      document.getElementById('next-button').style.display = 'block'; //after the game won, the next-button will show up and user can click on it to show the next image
    //music added when the game won, the applause sound
    var bMusic = new Audio('https://cdn.glitch.me/322c0444-bdb6-400c-8c7b-568c71c6bd72%2Fapplause%20audio.wav?v=1638555464182') //added cheering noises once game is won
	    bMusic.play()
...
function findAdjacencyDirection(tileLoc, nullLoc) {
  ...
  */
  if (tileLoc - nullLoc === 3) {
    ...
    return [1, 0];
  } else if (tileLoc - nullLoc === -1 && tileLoc % 3 !== 2) {  // Like above, we changed these numbers to 3's and 2's in order for our puzzle to work. 
    return [-1, 0];
  } else return null;
}
