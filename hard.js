//link for Inspiration: https://glitch.com/edit/#!/sliding-photo-puzzle
// javascript for hard game 


// tileState holds the game state at any point in time
const tileState = {
  // tileLoc will be set up as a hash of objects of the form {CSS id: tile position} where the locations are numbered 0 to 24.
  // nullLoc is the position of the gap, again numbers 0 through 24. Like our easy puzzle, these numbers are for the 5 by 5 grid 
  tileLoc: {},
  nullLoc: '',
};
...
function gameSetup() {
  const gameArea = document.getElementById('game-area-hard');
  const img = new Image();
  ...
  img.onload = function() {
    ...
    const tiles = [...Array(25)].map((_) => tileHTML); // The array is 25 because we want a 5 by 5 puzzle for the hard game
    ...
    document.getElementById('next-button').style.display = 'none'; //added the next-button, but it won't show up yet
  };
}
...
function tileSetup() {
  ...
  tileArray.forEach((tile, index) => {
    ...
    const backgroundPositionX = ((index % 5) * 99.6) / 4;
    const backgroundPositionY = (Math.floor(index / 5) * 99.6) / 4;
...
function startSwipe(e) {
  ...
  document.addEventListener(endType, function detectSwipeDirection(f) {
    let swipeDirection = [unify(f).clientX - unify(e).clientX, unify(f).clientY - unify(e).clientY];
    ...
    const denominator = Math.sqrt(swipeDirection[0] ** 2 + swipeDirection[1] ** 2);
...
function automaticMove() {
  if (document.querySelectorAll('.moving').length === 0) {
    ...
    for (let key in tileLocs) {
      if (findAdjacencyDirection(tileLocs[key], nullLoc)) validMoves[key] = tileLocs[key];
    }
    const candidateTileId = Object.keys(validMoves)[Math.floor(Math.random() * Object.keys(validMoves).length)];
...
function checkGameWon() {
  return gameWonState.started && Object.keys(tileState['tileLoc']).every((key) => tileState['tileLoc'][key] === gameWonState['tileLoc'][key]);
}
...
function makePlay(tileId, swipeDirection) {
  ...
  if (tileRelativePos) {
    if (
      ...
      (Math.abs(swipeDirection[0] + tileRelativePos[0]) < 0.5 && Math.abs(swipeDirection[1] + tileRelativePos[1]) < 0.5)
    ) {
  ...
  if (checkGameWon()) {
    document.getElementById('randomize-button').style.display = 'block';
    document.getElementById('next-button').style.display = 'block'; //after the game won, it will show up and user can click on it to the next image
    //music added when the game won, the applause sound
...
function moveTile(tileId, tileLoc, nullLoc) {
  ...
  const moveX = `calc(${direction[0] * -100}% + ${direction[0] * -5}px)`; //Like our easy puzzle, the last number was changed from 4 to 5
  const moveY = `calc(${direction[1] * -100}% + ${direction[1] * -5}px)`;
  ...
  const styleSheetIndex = Object.keys(document.styleSheets).find((key) => document.styleSheets[key].href.includes('/style.css'));
  ...
  tileEl.addEventListener('transitionend', function() {
    ...
    const styleRuleIndex = Object.keys(cssRules).find((key) => cssRules[key].selectorText === `#${tileId}.moving`);
    if (styleRuleIndex) {
...
function findAdjacencyDirection(tileLoc, nullLoc) {
  /* Return an x or y direction relative to the null space if the tile is adjacent, or if it isn't then return null
     ...
     [-1, 0]  null  [1, 0]
             [0, 1]
  */
  if (tileLoc - nullLoc === 5) {
    return [0, 1];
  } else if (tileLoc - nullLoc === -5) {
    return [0, -1];
  } else if (tileLoc - nullLoc === 1 && tileLoc % 5 !== 0) {
    return [1, 0];
  } else if (tileLoc - nullLoc === -1 && tileLoc % 5 !== 4) { //these numbers were also changed to work with the 5 by 5 grid
    return [-1, 0];
  } else return null;
}
...
function drawGame() {
  ...
  for (let key in tiles) {
    ...
    if (tileLoc >= 0) {
      ...
      const gridColumnStart = (tileLoc % 5) + 1;
      const gridRowStart = Math.floor(tileLoc / 5) + 1;
